...
def create_meme(cursor, image_id, top, bottom):
    cursor.execute('''INSERT INTO meme (image_id, top, bottom ) VALUES \
         (%d, '%s', '%s') RETURNING id''' % (image_id, html.escape(top), html.escape(bottom)))
...
def create_image(cursor, img_name):
    cursor.execute('''INSERT into image (name) values ('%s') RETURNING id''' % html.escape(img_name))
    return cursor.fetchone()[0]

def create_schema(cursor):
    cursor.execute('CREATE TABLE IF NOT EXISTS meme (id SERIAL PRIMARY KEY, image_id INTEGER, top text, bottom text);')
    cursor.execute('CREATE TABLE IF NOT EXISTS image (id SERIAL PRIMARY KEY, name text UNIQUE);')

    cursor.execute('''INSERT INTO image ( name ) VALUES ('aliens.jpg'),('awkwardpenguin.jpg'), \
         ('successkid.jpg'),('confessionbear.jpg'),('blb.png'),('bill-lumbergh.jpg'),('homer.gif') \
